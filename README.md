...
- **Dashboard of device tiles** — add each KVM by its local address (e.g. `workmac.local`) and launch it with a click instead of hunting for a browser tab.
...
1. On first launch you'll see an empty dashboard. Add a device using its local address (e.g. `workmac.local`).
